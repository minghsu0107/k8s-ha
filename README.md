...
However, HA architecture also brings about another problem - which master node replica should the client connect to? Simply adding mulitple DNS A records does not solve the problem since DNS server could not check server healthiness in real time.
